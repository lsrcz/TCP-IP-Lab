...
struct ErrorBehavior {
    ...
    inline ErrorBehavior(const char* msg, bool fatalFlag = false,
                         bool        haveAction = false,
                         ErrorSource from       = SOURCE_USER,
                         const char* sys_msg = "", pcap_t* pcap = NULL,
                         const char* buf = NULL)
        : msg(msg), fatalFlag(fatalFlag), haveAction(haveAction), from(from),
          sys_msg(sys_msg), pcap(pcap), buf(buf) {}
    inline ErrorBehavior& setFlag(bool fatalFlag, bool haveAction,
                                  ErrorSource from) {
...
template <typename T> inline bool isErrorBehavior(T t) {
    return false;
}

template <> inline bool isErrorBehavior(ErrorBehavior p) {
    return true;
}

template <typename T> inline ErrorBehavior behaviorFrom(T t) {
    return ErrorBehavior("");
}

template <> inline ErrorBehavior behaviorFrom(ErrorBehavior eb) {
    return eb;
}
...
#define ERROR_WITH_BEHAVIOR(behavior, action)                           \
    {                                                                   \
        ...
        case SOURCE_SYSTEM:                                             \
            LOG(_lv, std::string(_msg) + " \033[36m[" + _sys_msg + ": " \
                         + strerror(errno) + "]\033[0m");               \
            break;                                                      \
        case SOURCE_PCAP:                                               \
            LOG(_lv, std::string(_msg) + " \033[36m[" + _sys_msg + ": " \
                         + pcap_geterr((behavior).pcap) + "]\033[0m");  \
            break;                                                      \
        case SOURCE_USER:                                               \
