...
void RouterPort::initBDUpdateThread() {
    if (bdUpdateThread.joinable()) {
        ErrorBehavior eb("The BD update thread was initialized before...this should not happen", true, true);
...
void RouterPort::initHelloThread() {
    if (helloThread.joinable()) {
        ErrorBehavior eb("The hello thread was initialized before...this should not happen", true, true);
        ERROR_WITH_BEHAVIOR(eb, exit(-1));
    }
...
void RouterPort::initTaskThread() {
    if (taskThread.joinable()) {
        ErrorBehavior eb("The task thread was initialized before...this should not happen", true, true);
        ERROR_WITH_BEHAVIOR(eb, exit(-1));
    }
