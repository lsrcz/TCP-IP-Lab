...
void FrameDispatcher::addFrameDispatcher(uint16_t             protocol,
                                         frameReceiveCallback c) {
    std::unique_lock<std::shared_mutex> lock(mu);
    char                                buf[200];
    snprintf(buf, 200, "Registered dispatcher for protocol id 0x%04x",
             protocol);
    LOG(INFO, buf);
...
int FrameDispatcher::processFrame(const void* frame, int len, int id) {
    ...
    return (*(callbacks.find(ether_type))->second)(
        (uint8_t*)frame + ETHER_HDR_LEN, len - ETHER_HDR_LEN, id);
}
