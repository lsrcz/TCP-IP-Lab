...
void pcapHandler(u_char* user, const struct pcap_pkthdr* h,
                 const u_char* byte) {
...
bool device_t::init() {
    ...
    int* dlt_p;
    int  dlt_num =
        Pcap_list_datalinks(ppcap_dev.get(), &dlt_p, eb, return false);
...
auto device_list_t::getDeviceIter(std::string name) {
    return std::find_if(devices.begin(), devices.end(),
                        [&name](const device_t& d) { return d.name == name; });
}
auto device_list_t::getDeviceIter(int id) {
    return std::find_if(devices.begin(), devices.end(),
                        [id](const device_t& d) { return d.id == id; });
}
