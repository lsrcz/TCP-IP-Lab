...
int tcb::setOption(uint8_t* buf, int flags, uint32_t acktimestamp = 0, uint32_t ws = 0) {
    int b = 0;
    if (flags & TO_MSS) {
        b += setMSSOpt(buf + b);
    }
...
int tcb::tcpCopyToBuf(uint8_t* buf, tcphdr t, uint8_t* option, int optlen, uint8_t* data, int len) {
